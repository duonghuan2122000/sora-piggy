...
- **Acceptance Criteria**:
  - [ ] Add `import i18n from '@renderer/locales';` at the top of `language.ts`.
  - [ ] Remove the buggy code block accessing `window.$i18n`.
  - [ ] Update `i18n.global.locale.value = code;` in the `try` block (after DB save).
  - [ ] Update `i18n.global.locale.value = code;` in the `catch` block (to keep UI in sync on error).
  - [ ] Ensure database preference save logic (`window.api.setLanguagePreference`) remains unchanged.
